gadget_add_lengthgroups <- function (tracer_data, length_group, sigma_per_cohort) 
{
    if (length(length_group) < 2) {
        stop("Lengtht group should have at least 2 members")
    }
    tracer_data <- tracer_data[tracer_data$count > 0, ]
    tracer_data$length_var <- sigma_per_cohort[c(tracer_data$cohort + 1)]^2
