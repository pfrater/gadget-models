...
source('~/gadget/models/gadgetTest/results/surveyDistDataMerge.R')
ldist.spr <- 
...
# ldist.spr <-
#     ggplot(data=fit$out.fit$ldist.spr ,
#            aes(length,number)) + geom_line() +
#     geom_line(data=ldist.spr.data, aes(x=length, y=number), color='gray') +
#     facet_wrap(~year) + theme_bw() +
