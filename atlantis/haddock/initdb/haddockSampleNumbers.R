...
nlengths <- 
    translate.all.le() %>%
    filter(species.code == 2, sample.id %in% stations$sample.id) %>%
    left_join(stations) %>%
    group_by(year) %>%
...
nages <- 
    translate.all.kv() %>%
    filter(species.code == 2, sample.id %in% stations$sample.id, !is.na(age)) %>%
    left_join(stations) %>%
    group_by(year) %>%
    summarize(nAges = n())
...
fleet.le <-
    translate.all.le() %>%
    filter(species.code == 2, sample.id %in% comm.stations$sample.id) %>%
