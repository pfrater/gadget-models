getHaddockCatches <- function (adir, area_data, fishery) 
{
    nc_out <- ncdf4::nc_open(attr(adir, "nc_catch"))
    fishery_vars <- mfdbatlantis:::list_nc_variables(nc_out, "FHA_Catch")
    ...
    if (nrow(dims) == 0) 
        return(data.frame())
    data.frame(area = dims$area, time = dims$time, year = mfdbatlantis:::atlantis_time_to_years(dims$time) + 
