...
init.params[grep('init.abund', rn), ] <- c('cod.init.abund', 10, 1e-05, 100, 1)
init.params[grep('rec.sd', rn), ] <- c('cod.rec.sd', 2, 0.1, 10, 1)
init.params[grep('rec.mult', rn), ] <- c('cod.rec.mult', 100, 1e-05, 500, 1)
init.params[grep('cod.mort', rn), ] <- c('cod.mort', 0.2, 0.15, 0.4, 1)
...
init.params[grepl('l50',init.params$switch),'value'] <- 65
...
init.params[init.params$switch=='igfs.alpha',] <- c('igfs.alpha', 0.05, 0.001, 3, 1)
init.params[init.params$switch=='igfs.l50',] <- c('igfs.l50', 50, 5, 55, 1)
init.params[init.params$switch=='aut.alpha',] <- c('aut.alpha', 0.05, 0.001, 3, 1)
init.params[init.params$switch=='aut.l50',] <- c('aut.l50', 50, 5, 55, 1)
