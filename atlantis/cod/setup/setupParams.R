...
read.gadget.parameters(sprintf('%s/params.out', gd$dir)) %>%
    ...
    init.params('spawn.alpha', 0.05, 0.001, 3, 1) %>%
    init.params('spawn.l50', 50, 5, 120, 1) %>%
    init.params('wl.alpha', 0.05, 0.001, 3, 1) %>%
    init.params('wl.l50', 50, 5, 120, 1) %>%
    init.params('bh.mu', 5e+06, 0, 1e+10, 1) %>%
    init.params('bh.lam', 1e+06, 10, 2e+07, 1) %>%
