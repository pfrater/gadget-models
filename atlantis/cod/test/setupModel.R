...
cod0 <- 
    ...
    gadget_update('doesgrow',
                  growthparameters=c(linf=sprintf('#%s.linf', species.name), 
                                     ...
                                     alpha=weight.alpha,
                                     beta=weight.beta),
                  beta=sprintf('(* 10 #%s.bbin)', stocknames)) %>%
    ...
    gadget_update('naturalmortality', # m as a function of age
                  m.estimate.formula(age=.[[1]]$minage:.[[1]]$maxage,
                                     m=sprintf('%s.m.decay', stocknames),
                                     max.m=sprintf('%s.max.m', stocknames),
                                     min.m=sprintf('%s.min.m', stocknames))) %>%
                  ...
                  normalparam=
                      data_frame(age = .[[1]]$minage:.[[1]]$maxage, 
                                 ...
                                 age.factor=init.age.factor(age=age,
                                                            m=sprintf('%s.init.decay', stocknames),
                                                            age.scalar=sprintf('%s.init.scalar', stocknames),
                                                            init.min=sprintf('%s.init.min', stocknames)),
