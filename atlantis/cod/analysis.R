...
init.plot <- 
    ggplot(data=data.frame(age=unique(fit$stock.std$age)),
           aes(x=age)) + 
    stat_function(fun=function(x, m.decay, init.scalar, init.min) {
                exp((-1)*m.decay*x)*(init.scalar - init.min) + init.min
            }, args=list(m.decay=fit$params$value[grep('m.decay', 
                                                     fit$params$switch)],
                         init.scalar=fit$params$value[grep('init.scalar', 
                                                     fit$params$switch)],
                         init.min=fit$params$value[grep('init.min', 
                                                           fit$params$switch)]
                ))