...
bs_models <- list.dirs("errBootRuns/bootrun_var0.147/BS.WGTS", 
                       full.names = FALSE, recursive = FALSE)

boot_fit <- lapply(bs_models, function(x) {
    load(paste(basedir, "errBootRuns/bootrun_var0.147", "BS.WGTS", x, 
...
bootModelsPlot <- 
    ggplot(data=half_boot_res_by_year, aes(x=year, y=total.number/1e6, 
                                     color=boot_iter)) + 
...
bootModShadePlot <- 
    ggplot(data=half_boot_summary, aes(x = year, 
...
save(half_boot_res_by_year, file = "results/halfErrBootMod/halfErrBootFit.Rdata")
save(half_boot_summary, file = "results/halfErrBootMod/halfErrBootSummary.Rdata")
