...
## putting these m values to a function to obtain theoretical estimates
#mortality.function <- function(age, b, c) {
#    return(exp((-age*b) - c))
#}

#m.func.optimizer <- function(data, age) {
#    b <- data[1];
#    c <- data[2];
#    return(mortality.function(age, b, c));
#}
#
#m.func.sse <- function(data, age, vals) {
#    m.hat <- m.func.optimizer(data, age);
#    return(sum((vals - m.hat)^2))
#}
#
#
#params <- nlm(m.func.sse, c(0.2, 1.5), age=test2$age, vals=test2$m)
